...
<link rel="stylesheet" type="text/css"
	href="<c:url value='/styles/allMessages.css'/>">
<link rel="stylesheet"
...
<body>
	...
	<div class="container">
			...
			<c:forEach items="${messages}" var="m">
				<div class="message">
					<div class="message-content">
						<p>
							<strong>Od:</strong> ${m.sender.name } | <strong>Za:</strong>
							${m.receiver.name }
						</p>
						<p>
							<strong>Tekst:</strong> ${m.content} | <strong>Vreme:</strong>
							${m.timestamp }
						</p>
					</div>
					<div class="delete-icon">
						<a href="/Polovni/admin/deleteMessage?idMessage=${m.idMessage}"
							title="Obrisi poruku"> <i class="fas fa-trash-alt"></i>
						</a>
					</div>
				</div>
			</c:forEach>
		...
		<c:if test="${empty messages}">
			<p style="text-align: center; color: #7f8c8d;">No messages to
				display.</p>
		</c:if>
	</div>
